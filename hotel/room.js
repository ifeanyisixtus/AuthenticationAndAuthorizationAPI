...
app.post("/api/v1/rooms", async (req, res) => {
  ...
  try {
    res
      .status(200)
      .send({ message: MESSAGES.ROOM.CREATED, success: true, room });
  } catch (err) {
    res
      .status(500)
...
const url =
  "/api/v1/rooms?search={searchRoomNameMatch}&roomType={searchRoomTypeNameMatch}&minPrice={searchRoomMinimumPriceMatch}&maxPrice={searchRoomMaximumPriceMatch}";
...
app.patch("/api/v1/rooms/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .send({ message: MESSAGES.ROOM.UPDATED, success: true, room });
  } catch (err) {
    res
      .status(500)
      .send({ message: err.message || MESSAGES.ERROR, success: false });
...
app.delete("/api/v1/rooms/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .send({ message: MESSAGES.ROOM.DELETED, success: true, room });
  } catch (err) {
    res
      .status(500)
      .send({ message: err.message || MESSAGES.ERROR, success: false });
...
app.get("/api/v1/rooms/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .send({ message: MESSAGES.ROOM.FETCHED, success: true, room });
  } catch (err) {
    res
      .status(500)
      .send({ message: err.message || MESSAGES.ERROR, success: false });
